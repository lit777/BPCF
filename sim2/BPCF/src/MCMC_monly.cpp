...
List MCMC(
    ...
    NumericVector M1(n), M0(n), Y1(n), Y0(n);
    NumericVector Xcut_ps[P + 1], Xcut[P], Xcut_y[2]; // e.g. unique value of potential confounders
    for (int j = 0; j < P; j++)
    {
        NumericVector temp;
...
//    NumericVector R  = clone(Z);
  ...
  //  NumericVector post_dir_alpha  = rep(1.0, P);
    ...
    NumericVector post_dir_alpha4 = rep(1.0, 2);
...
//    NumericMatrix Tree   (n,     m);
    ...
    NumericMatrix TreeY_holder  (1,    2);
...
//    NumericVector prop_prob = rdirichlet(1, rep(1, P));
    ...
    NumericVector prop_prob4 = rdirichlet(1, rep(1, 2));
    
    NumericMatrix xpred_mult(n, P + 1), Xpred1(n, P + 1), Xpred2(n, 2), prop_Xpred2(n, 2);   // placeholder for bootstrap sample for inference (correct this statement)
    ...
    for( int i = 1; i < n; i++)
    {
       ...
       xpred_mult(i, P) = i;
       Xpred1(i, P) = PS(i);
       Xpred2(i, 0) = Rcpp::rnorm(1, mean(M_out)+mshift, 1)(0);
       Xpred2(i, 1) = Rcpp::rnorm(1, mean(M_out)+mshift, 1)(0);
       prop_Xpred2(i, 0) = Rcpp::rnorm(1, mean(M_out)+mshift, 1)(0);
       prop_Xpred2(i, 1) = Rcpp::rnorm(1, mean(M_out)+mshift, 1)(0);
    }
    ...
    for (int iter = 1; iter <= n_iter; iter++)
    {
        ...
        if (iter > 1)
        {
          for (int i = 0; i < n; i++)
          {
            ...
            if (Y_trt(i) == 1)
            {
              ...
              prop_Xpred2(i, 0) = clone(prop_M)(0);
              prop_Xpred2(i, 1) = M_out(i) + mshift;
            }
            else
            {
              prop_M = Rcpp::rnorm(1, mu1, 0.5);
              prop_Xpred2(i, 0) = M_out(i) + mshift;
              prop_Xpred2(i, 1) = clone(prop_M)(0); 
            }
            ...
            if (ratio > log(R::runif(0, 1)))
            {
              M1(i) = clone(prop_Xpred2)(i, 1);
              M0(i) = clone(prop_Xpred2)(i, 0);
            //  Rcout << "Rcpp accept: " << "yes" <<  std::endl;
            }
            Xpred2(i, 0) = clone(M0)(i);
            Xpred2(i, 1) = clone(M1)(i);
          }
        ...
        NumericVector add1(P+1), add2(P), add3(P+1), add4(2);
