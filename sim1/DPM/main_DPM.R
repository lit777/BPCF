...
Xpred <- do.call(cbind, cov) # vec. of predictors

reg1 <- ifelse(Xpred[,1]< 0, 1, -1 )
reg2 <- ifelse(Xpred[,2]< 0, -1, 1 )

A <- rbinom(n, 1, pnorm(0.5+reg1+reg2-0.5*abs(Xpred[,3]-1)+1.5*Xpred[,4]*Xpred[,5])) # exposure model
...
Y_out <- ifelse(A==1, Y_1, Y_0)     # observed outcome
...
save(Y_1, Y_0, M_1, M_0, A,output, file=paste0("Sim1_DP_",process,".RData"))
  }, error=function(e){})
}
