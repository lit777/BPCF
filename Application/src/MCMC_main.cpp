...
List MCMC(
    ...
    // Initial values of residuals
    NumericVector R  = clone(Z);
    NumericVector R_M = Rcpp::rnorm(n, 0, 1);
    NumericVector R_Y = Rcpp::rnorm(n, 0, 1);
    
    // Initial values for the selection probabilities (now, we just use uniform prob)
    ...
    NumericMatrix xpred_mult(n, P + 1), Xpred1(n, P + 1), Xpred2(n, P + 2), prop_Xpred2(n, P + 2);   // placeholder for inference 
    ...
    for (int iter = 1; iter <= n_iter; iter++)
    {
        ...
        // impute counterfactual intermediate variables
        ...
        if (iter > 1)
        {
          for (int i = 0; i < n; i++)
          {
            ...
            {
              dir_lik_p = R::dnorm(M1(i), mu1, 0.5, true)+R::dnorm(prop_M(0), mu1, sqrt(sigma2_m(iter - 1)), true )+R::dnorm(Y_out(i), sum(clone(Tree3)(i,_))+sum(clone(TreeY))*(Y_trt(i)-0.5), sqrt(sigma2_y(iter - 1)), true);
              dir_lik = R::dnorm(prop_M(0), mu1, 0.5, true)+R::dnorm(M1(i), mu1, sqrt(sigma2_m(iter - 1)), true )+R::dnorm(Y_out(i), sum(clone(Tree3)(i,_))+sum(clone(Tree4)(i,_))*(Y_trt(i)-0.5), sqrt(sigma2_y(iter - 1)), true);
              ratio = dir_lik_p - dir_lik;
        ...
        {
          log_Y(i) = R::dnorm(Y_out(i), sum(clone(Tree3)(i,_))+sum(clone(Tree4)(i,_))*(Y_trt(i)-0.5), sqrt(sigma2_y(iter)), true);
        }
        LOG_Y = sum(log_Y);
        ...
        // in fact, we use uniform prior on selection probabilities
