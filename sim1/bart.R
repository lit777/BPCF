...
for(test_case in 1:200) {
  ...
  DE <- AE1 <- AE2 <- NULL
  for(i in 1:500){
    DE[i] <- mean(rcpp$predicted_Y1[abs(rcpp$predicted_M1[,i]-rcpp$predicted_M0[,i])<0.5,i]-rcpp$predicted_Y0[abs(rcpp$predicted_M1[,i]-rcpp$predicted_M0[,i])<0.5,i])
    AE2[i] <- mean(rcpp$predicted_Y1[(rcpp$predicted_M1[,i]-rcpp$predicted_M0[,i])< -0.5,i]-rcpp$predicted_Y0[(rcpp$predicted_M1[,i]-rcpp$predicted_M0[,i])< -0.5,i])
    AE1[i] <- mean(rcpp$predicted_Y1[(rcpp$predicted_M1[,i]-rcpp$predicted_M0[,i])> 0.5,i]-rcpp$predicted_Y0[(rcpp$predicted_M1[,i]-rcpp$predicted_M0[,i])> 0.5,i])
  }
