...
List MCMC(
    ...
    const int m,           // Num. of Trees: default setting 200
    const int m1,          // Num. of Trees: default setting 200
    const int m2,          // Num. of Trees: default setting 50
    const int m3,          // Num. of Trees: default setting 200
    const int m4,          // Num. of Trees: default setting 50
    const int nu,
    ...
    NumericVector sp,
    NumericMatrix Wtriplet,
    NumericVector Wtripletsum,
    NumericMatrix Wbegfin,
    double rho,
    const double tau2_posterior_shape,
    const NumericVector prior_tau2,
    ...
    NumericVector M1(n), M0(n), Y1(n), Y0(n), ePS(n);
    ...
    double tau2 = 0.1; // initial
    
    // a quadratic form for the random effects
    ...
    // Run MCMC for PS
    for (int iter = 1; iter <= n_iter_ps; iter++){
      ...
      {
        log_Z(i) = R::dnorm(Z(i), rowSums(Tree)(i), 1, true);
      }
      LOG_Z = sum(log_Z);
      
      ePS = Rcpp::pnorm(rowSums(Tree),0, 1, true, false);
      ...
      if (iter > burn_in_ps)
      {
        ...
        {
          thin_count = 1;
          predicted_PS (_, post_sample_idx) = ePS;
          post_sample_idx++;
        }
      }
    ...
    for (int iter = 1; iter <= n_iter; iter++)
    {
        ...
        if (iter % 100 == 0)
        {
          for (int i = 0; i < n; i++)
          {
            Xpred1(i, P) = predicted_PS(i,ps_count);
          }
         ps_count++;
        } 
        ...
        if (iter > 1)
        {
          for (int i = 0; i < n; i++)
          {
            ...
            if (Y_trt(i) == 1)
            {
              dir_lik_p = R::dnorm(M0(i), mu0, 0.5, true)+R::dnorm(prop_M(0), mu0, sqrt(sigma2_m(iter - 1)), true )+R::dnorm(Y_out(i), sum(clone(Tree3)(i,_))+sum(clone(TreeY))*(Y_trt(i)-0.5), sqrt(sigma2_y(iter - 1)), true);
              dir_lik = R::dnorm(prop_M(0), mu0, 0.5, true)+R::dnorm(M0(i), mu0, sqrt(sigma2_m(iter - 1)), true )+R::dnorm(Y_out(i), sum(clone(Tree3)(i,_))+sum(clone(Tree4)(i,_))*(Y_trt(i)-0.5), sqrt(sigma2_y(iter - 1)), true);
              ratio = dir_lik_p - dir_lik;
            }
            else
            {
              dir_lik_p = R::dnorm(M1(i), mu1, 0.5, true)+R::dnorm(prop_M(0), mu1, sqrt(sigma2_m(iter - 1)), true )+R::dnorm(Y_out(i), sum(clone(Tree3)(i,_))+sum(clone(TreeY))*(Y_trt(i)-0.5)+sp(i), sqrt(sigma2_y(iter - 1)), true);
              dir_lik = R::dnorm(prop_M(0), mu1, 0.5, true)+R::dnorm(M1(i), mu1, sqrt(sigma2_m(iter - 1)), true )+R::dnorm(Y_out(i), sum(clone(Tree3)(i,_))+sum(clone(Tree4)(i,_))*(Y_trt(i)-0.5)+sp(i), sqrt(sigma2_y(iter - 1)), true);
              ratio = dir_lik_p - dir_lik;
        ...
        {
          log_M(i) = R::dnorm(M_out(i), sum(clone(Tree1)(i,_))+sum(clone(Tree2)(i,_))*(Y_trt(i)-0.5), sqrt(sigma2_m(iter)), true);
        }
        LOG_M = sum(log_M);
        ...
        for (int t = 0; t < m3; t++)
        {
          update_R_mu(R_Y, Y_out - sp, Tree3, Tree4, Y_trt, t);
        ...
        for (int t = 0; t < m4; t++)
        {
          update_R_tau(R_Y, Y_out - sp, Tree3, Tree4, Y_trt, t);
        ...
        for (int i = 0; i < n; i++)
        {
          Y1(i) = Y_trt(i) * Y_out(i) + (1 - Y_trt(i)) * (clone(rowSums(Tree4))(i) + Y_out(i) ) + sp(i) + shift;
          Y0(i) = (1 - Y_trt(i)) * Y_out(i) + Y_trt(i) * (Y_out(i) - clone(rowSums(Tree4))(i) ) + sp(i) + shift;
        }
        
        NumericVector y_new = clone(rowSums(Tree3)) + clone(rowSums(Tree4)) * (Y_trt - 0.5) + sp;
        ...
        offset = Y_out-clone(rowSums(Tree3)) - clone(rowSums(Tree4)) * (Y_trt - 0.5);
        ...
        for(int j = 0; j < n; j++)
        {
          ...
          rowend = Wbegfin(j,1);
          sumphi = 0;
          for(int l = rowstart; l < rowend; l++) sumphi += Wtriplet(l, 2) * sp[(Wtriplet(l,1) - 1)];
          priormean = rho * sumphi / priorvardenom;
          
          // propose a value
          fcprecision = (1/sigma2_y(iter)) + (1/priorvar);
          fcsd = pow((1/fcprecision),0.5);
          fcmean = (priormean / priorvar + offset[j]/sigma2_y(iter)) / fcprecision;
          sp[j] = rnorm(1, fcmean, fcsd)[0];
        }
        sp = sp - mean(sp);
        ...
        for(int l = 0; l < n; l++)
        {
          ...
          if(row == col) {
            ...
            tau2_quadform = tau2_quadform + sp[(Wtriplet(l,0) - 1)] * sp[(Wtriplet(l,1) - 1)] * Wtriplet(l,2);
          }
        }
        ...
        for(int l = 0; l < n; l++)
        {
          tau2_phisq = tau2_phisq + sp[l] * sp[l] * (rho * Wtripletsum[l] + 1 - rho);
        }
        
        // Compute the quadratic form
        tau2_posteriorscale = 0.5 * (tau2_phisq - rho * tau2_quadform) + prior_tau2(1);
        NumericVector temp = rinvgamma(1, tau2_posterior_shape, tau2_posteriorscale);
        tau2 = temp(0);
        
        for (int i = 0; i < Tree3.nrow(); i++)
        {
          log_Y(i) = R::dnorm(Y_out(i), sum(clone(Tree3)(i,_))+sum(clone(Tree4)(i,_))*(Y_trt(i)-0.5)+sp(i), sqrt(sigma2_y(iter)), true);
        }
        LOG_Y = sum(log_Y);
        ...
        log_marginal(iter-1) = LOG_Z+LOG_M+LOG_Y;
