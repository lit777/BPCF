...
M_err <- rnorm(n, 0, 0.1)
M_1 <- -0.5*reg1+0.5*reg2+1*1.3+1*abs(Xpred[,3]+1) + 1.5*Xpred[,4] - exp(0.3*Xpred[,5]) - 1.5*abs(Xpred[,5]+0.3) + M_err
M_0 <- -0.5*reg1+0.5*reg2+(0)*1.3+1*abs(Xpred[,3]+1) + 1.5*Xpred[,4] - exp(0.3*Xpred[,5]) - (0)*abs(Xpred[,5]+0.3) + M_err
M_out <- ifelse(Y_trt==1, M_1, M_0)

S <- M_1-M_0

Y_err <- rnorm(n, 0, 0.3)
Y_1 <- 1*reg1+1.5*reg2-1+2*abs(Xpred[,3]+1)  -1.5*(S+2)^2 + 2*Xpred[,4]+ exp(0.5*Xpred[,5])-0.5*1*abs(Xpred[,6]) - 1*1*abs(Xpred[,7]+1)+Y_err
Y_0 <- 1*reg1+1.5*reg2-(0)+2*abs(Xpred[,3]+1) -0.3*(S+2)^2 + 2*Xpred[,4]+ exp(0.5*Xpred[,5])-0.5*(0)*abs(Xpred[,6]) - 1*(0)*abs(Xpred[,7]+1)+Y_err
